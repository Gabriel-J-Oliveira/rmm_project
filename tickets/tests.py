...
class TicketCentralTests(DeskTechnicalTestCase):
    ...
    def test_new_query_opens_quick_ticket_drawer_without_prefill(self):
        response = self.client.get(reverse('tickets:central'), {'new': '1'}, HTTP_HOST=self.host)

        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'data-quick-ticket-drawer')
        self.assertContains(response, "new URLSearchParams(window.location.search).get('new') === '1'")
        self.assertContains(response, 'setTimeout(() => openQuickTicketDrawer(), 120)')
